...
int main( int argc, char *argv[] ){
  ...
  double *vector, *vector_total_manual_allreduce, *vector_total_allreduce, *buf_send, *buf_recv, *buf_temp;
  ...
  for( count = 1; count < nprocs; count++ ){
    ...
    if( count != 1 ) for( i = 0; i < SIZE; i++ ) vector_total_manual_allreduce[i] += buf_recv[i];
    else for( i = 0; i < SIZE; i++ ) vector_total_manual_allreduce[i] += vector[i] + buf_recv[i];

    MPI_Wait( &request, MPI_STATUS_IGNORE );

    if( count != 1 ) buf_temp = buf_send;
    buf_send = buf_recv;
    buf_recv = buf_temp;

...
int main(int argc, char * argv[])
{
  ...
  const int N = 3;     // Global dimension
  ...
  int * loc_A_budw = malloc(N*n*sizeof(int));
  int * loc_A_buup = malloc(N*n*sizeof(int));
...
//  MPI_Sendrecv(loc_A, n * N, MPI_INT, PREC, rank , loc_A_buff, n * N,
  //     MPI_INT, NEXT, NEXT, MPI_COMM_WORLD, MPI_STATUS_IGNORE );
    ...
    MPI_Sendrecv(loc_A, n * N, MPI_INT, PREC, rank , loc_A_buup, n * N,
         MPI_INT, PREC, PREC, MPI_COMM_WORLD, MPI_STATUS_IGNORE );

    MPI_Sendrecv(loc_A, n * N, MPI_INT, NEXT, rank , loc_A_budw, n * N,
        MPI_INT, NEXT, NEXT, MPI_COMM_WORLD, MPI_STATUS_IGNORE );
